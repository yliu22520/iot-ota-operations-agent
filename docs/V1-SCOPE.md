...
- 首轮真实模型评测使用 DeepSeek API。
- 两个候选固定为 `deepseek-v4-flash` 与 `deepseek-v4-pro`，使用相同 Prompt、工具和 15 个评测案例比较。
- Spring AI 使用原生 `spring-ai-starter-model-deepseek` 接入，不把 DeepSeek 凭证配置成 OpenAI 凭证。
- API 密钥仅通过服务端环境变量 `DEEPSEEK_API_KEY` 注入，不进入源码、配置文件、文档、日志或版本库。
- 首轮评测使用模型默认思考模式；`reasoning_content` 只在供应商协议要求的工具调用过程中短暂传递，任务结束后丢弃，不持久化、不记录日志、不对任何角色展示。
...
- 云端仅通过平台 Secret 或环境变量注入 `DEEPSEEK_API_KEY`；密钥不进入 Docker Compose、服务器脚本、镜像或版本库，轮换后通过重启应用生效。
...
- 每个垂直切片单独提交代码、测试、迁移和必要文档；CI 永远不调用真实 DeepSeek API。
